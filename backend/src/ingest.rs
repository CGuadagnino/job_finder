...
pub struct AdzunaJob {
    ...
    pub redirect_url: String, // Note: they use redirect_url, not url
...
impl From<AdzunaJob> for NewJob {
    fn from(adzuna_job: AdzunaJob) -> Self {
        NewJob {
            ...
            url: adzuna_job.redirect_url, // Map redirect_url -> url
...
pub async fn fetch_jobs_from_adzuna(
    ...
    let new_jobs: Vec<NewJob> = response
        .results
        .into_iter()
        .map(|job| NewJob::from(job))
        .collect();
